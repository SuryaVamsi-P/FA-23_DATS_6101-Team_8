...
names(training_data3) <- make.names(names(training_data3), unique = TRUE)
names(testing_data3) <- make.names(names(testing_data3), unique = TRUE)

# Check for NA values
if (any(is.na(training_data3)) || any(is.na(testing_data3))) {
...
predictions <- predict(tuned_model, newdata = testing_data3)

# Create a confusion matrix
conf_matrix <- confusionMatrix(predictions, testing_data3$hospital_death)
...
prob_predictions <- predict(tuned_model, newdata = testing_data3, type = "prob")
...
roc_result <- roc(testing_data3$hospital_death, prob_positive_class)
